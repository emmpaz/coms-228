...
public abstract class Sorter
{
  ...
  public void sortWithStatistics(WordList toSort, Comparator<String> comp, int totalToSort) throws NullPointerException, IllegalArgumentException{
      CountingComparator<String> counter = new CountingComparator<>(comp);
      if(totalToSort == 0){
      ...
      try {
        WordList clone = toSort.clone();
        sort(clone, comp);
        FileWriter writeTo = new FileWriter(getName() + ".txt");
        for(String i : clone.getArray()){
          writeTo.write(i);
          writeTo.write("\n");
        }
        writeTo.close();
