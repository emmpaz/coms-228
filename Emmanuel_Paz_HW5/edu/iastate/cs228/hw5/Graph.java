...
public class Graph {
  ...
  {
    if(fromIdx > vertices.length-1 || fromIdx < 0 || toIdx > vertices.length-1 || toIdx < 0)
      throw new IndexOutOfBoundsException();
    if(word == null)
      throw new NullPointerException();
    Vertex from = vertices[fromIdx];
    Vertex to = vertices[toIdx];
    from.edges().add(new GraphEdge(index, word, from, to));
    to.edges().add(new GraphEdge(index, word, to, from));
  }
  ...
  private class GraphVertex implements Vertex
  {
    ...
    /**
     * this method just returns if the vertex has been visited
     * @return if visited
     */
    @Override
    ...
    /**
     * sets the variable to if true or false
    ...
    /**
     * This recursion method goes through the neighbors of the vertex and checks if they have been visited
     * @param from the vertex
     *   the vertex from which this vertex was visited
     * @return true if vertex/edges have a cycle
     */
    @Override
    public boolean hasCycle(Vertex from)
    {
      if(this.isVisited())
        ...
        for(Edge i : this.edges){
          if(!i.getTo().equals(from)){
            if(i.getTo().hasCycle(this))
              return true;
          }
        }
      }
