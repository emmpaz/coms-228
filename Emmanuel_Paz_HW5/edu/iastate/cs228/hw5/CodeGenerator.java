...
public class CodeGenerator
{
  ...
  private void array(PrintStream ps, String[] data, String access, String varName)
  {
    ...
    for(int i = 0; i< data.length-1;i++){
      data[i] = specialChar(data[i]) + "";
      ps.printf("\"%s\", ",data[i]);
    }
    ps.printf("\"%s\"", data[data.length-1]);
    ps.print("};");
    ps.println();
  ...
  /**
   * This is a helper method to deal with characters that would break the code (like " and \)
   * @param toScan the string we are scanning
   * @return the correctly formed string
   */
  private String specialChar(String toScan){
    String temp = "";
    for(int i = 0; i < toScan.length(); i++){
      if(toScan.charAt(i) == '"')
        temp += "\\";
      else if(toScan.charAt(i)== '\\')
        temp += "\\";
      temp += toScan.charAt(i);
    }
    return temp;
  }
  /**
  ...
  private void table(PrintStream ps, int[][] data, String access, String varName)
  {
    ...
    for(int i = 0; i < data.length-1; i++){
      ...
      ps.print(data[i][data[i].length-1]);
      ps.print("}, ");
    }
    ps.print("{");
    for(int i = 0; i < data[data.length-1].length-1; i++){
     ps.print(data[data.length-1][i] + ", ");
    }
    ps.print(data[data.length-1][data[data.length-1].length-1]);
    ps.print("}");
    ps.print("};");
    ps.println();
